...
const Testimonials = () => {
  ...
  useEffect(() => {
    fetch("/api/feedback/get")
      ...
      .then((data) => {
        ...
        setTestimonials(
          data.feedbacks
            .filter((fb) => (fb.rating || 0) >= 4 && fb.feedback?.trim())
  ...
  const cards = testimonials.map((fb, i) => (
    ...
    >
      <p className="text-gray-light-2 leading-relaxed relative z-10 line-clamp-4">
        “{fb.feedback}”
      </p>
  ...
  return (
    <section className="w-full relative select-none py-16">
      ...
      <div
        ...
        style={{ "--marquee-duration": `${Math.max(testimonials.length * 6, 24)}s` }}
      >
